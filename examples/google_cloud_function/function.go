...
// notBefore returns the time when the installation access token becomes valid.
func (iat *installationAccessToken) notBefore() time.Time {
	return iat.ExpiresAt.Add(-1 * time.Hour)
}

// notAfter returns the time when the installation access token expires.
func (iat *installationAccessToken) notAfter() time.Time {
	return iat.ExpiresAt
}

// verify checks the validity of the installation access token.
